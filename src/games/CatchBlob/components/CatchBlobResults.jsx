      ...
      <div style={S.body}>
        ...
        >
          ...
          <GradientTitle as="h2" size="lg" gradient={C.gradient}>
            {isFinal ? 'Maestro del Cesto' : 'Risultati'}
          </GradientTitle>
        </motion.div>
          ...
          {sorted.map((p, i) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ ...S.lbScore, color: C.accent }}>{score}pt</span>
              </motion.div>
            )
          })}
        </div>
...
const S = {
  ...
  colScore: { fontSize: 'clamp(12px, 1.5dvh, 15px)', fontWeight: 900 },
  leaderboard: { display: 'flex', flexDirection: 'column', gap: 'clamp(4px, 0.6dvh, 6px)' },
  lbRow: {
    display: 'flex',
    alignItems: 'center',
  ...
  lbRank: { fontSize: 'clamp(12px, 1.4dvh, 14px)', fontWeight: 800, minWidth: 24, textAlign: 'center' },
  lbDot: { width: 16, height: 16, borderRadius: '50%', flexShrink: 0 },
  lbName: { flex: 1, fontWeight: 600, fontSize: 'clamp(12px, 1.4dvh, 14px)' },
  lbScore: { fontWeight: 800, fontSize: 'clamp(13px, 1.6dvh, 17px)', minWidth: 36, textAlign: 'right' },
  footer: { marginTop: 'auto', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8 },
  waitText: {
    color: 'var(--muted)',
...
export default CatchBlobResults
