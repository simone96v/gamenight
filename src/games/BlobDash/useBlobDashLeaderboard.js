// Hook + helper per la classifica globale Blob Dash.
// Persistenza: Supabase table public.blobdash_scores (one row per device, best score).
// Stesso pattern di useBlobJumpLeaderboard / useFlappyBlobLeaderboard.

import { useCallback, useEffect, useState } from 'react'
import { supabase } from '../../lib/supabase'
import { getDeviceId } from '../../utils/device'

const TABLE = 'blobdash_scores'
const TOP_LIMIT = 20

export async function submitBlobDashScore({ score, playerName, color, source = 'solo' }) {
...
export async function fetchTopBlobDashScores(limit = TOP_LIMIT) {
...
export function useBlobDashLeaderboard({ enabled = true } = {}) {
